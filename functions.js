...
function sortBucket(basket) {
  return basket.sort(function(first, second) {
...
function processBookWithMostCopies(bookBasket) {
...
function findBestPositionForBook(currentBook, currentCopy, arrayOfSets) {
  ...
  for (var k = 0; k < arrayOfSets.length; k++) {
    ...
    // console.log(`Testing copy ${currentCopy} of book ${currentBook[book]} inside set ${k}: ${tempArrayOfSets[k]}`);
...
function process(bookBasket) {
  ...
  let arrayOfSets = processBookWithMostCopies(sortedBookBasket);
...
module.exports = {
  ...
  processBookWithMostCopies: processBookWithMostCopies,
  processBooksWithSameNumberOfCopies: processBooksWithSameNumberOfCopies,
  findBestPositionForBook: findBestPositionForBook,
  processOtherBooks: processOtherBooks,
  process: process
}
